...
class StatisticsLibrary extends Model
{
    ...
    public function getAllUserStatsData()
    {
        return $this->db->queryFetchAll(
            ...
            INNER JOIN ' . USERS . ' AS u
                ON us.`user_statistic_user_id` = u.`user_id` AND u.`user_authlevel` <= ' . Functions::readConfig('stat_admin_level') . ' AND u.`user_is_bot` = 0
    ...
    public function getAllAllianceStatsData()
    {
        return $this->db->queryFetchAll(
            ...
            FROM ' . ALLIANCE . ' AS a
            INNER JOIN ' . USERS . ' AS u
                ON a.`alliance_id` = u.`user_ally_id` AND u.`user_authlevel` <= ' . Functions::readConfig('stat_admin_level') . ' AND u.`user_is_bot` = 0
